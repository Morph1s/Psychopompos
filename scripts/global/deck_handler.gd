...
func get_card_selection(amount: int, rarity_dist: Dictionary = rarity_distribution) -> Array[CardType]:	
	...
	for i in amount:
		...
		match card_rarity:
			...
			CardType.Rarity.GODS_BOON:
				selected_god_cards.append(card_library.gods_boon_cards.pop_at(card_index))
	...
	if selected_god_cards:
		card_library.gods_boon_cards.append_array(selected_god_cards)
	
	return selected_common_cards + selected_hero_cards + selected_god_cards
